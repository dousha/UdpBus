...
public class Hub
{
    ...
    private void Start()
    {
        ...
        foreach (var busWorker in upstream) busWorker.Start();

        foreach (var busWorker in downstream) busWorker.Start();
    ...
    public void Stop()
    {
        ...
        foreach (var busWorker in downstream) busWorker.Stop();

        foreach (var busWorker in upstream) busWorker.Stop();
    ...
    private void ProcessDataTransfer()
    {
        while (true)
        {
            ...
            switch (datagram.Direction)
            {
                ...
                {
                    foreach (var busWorker in downstream) busWorker.Send(datagram);

                    break;
                }
                case DatagramDirection.ToUpstream:
                {
                    foreach (var busWorker in upstream) busWorker.Send(datagram);
